...
public class PumpSensorQuantilesFunction extends AbstractSensorQuantilesFunction<PumpSensor> {

	@Override
	public Void process(PumpSensor sensor, Context ctx) throws Exception {
		if (!initialized) {
			init(ctx);
		}
	...
	protected void init(Context ctx) {
		super.init(ctx);
		PumpSensor.getClassSchema().getFields().forEach(field -> {
